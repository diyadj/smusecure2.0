...
def get_document_vectors(df, text_column):
    """Generate document vectors using TF-IDF"""
    print("Generating document vectors using TF-IDF...")
    ...
    doc_vectors = tfidf.fit_transform(df[text_column])
    return doc_vectors.toarray()

def perform_clustering(doc_vectors, n_clusters=5):
...
def analyze_news_articles(df, text_column='clean_text', n_clusters=5, n_topics=5):
    ...
    try:
        ...
        if df[text_column].isnull().any():
            print("Warning: Found null values in text column. Removing...")
        ...
        doc_vectors = get_document_vectors(df, text_column)
